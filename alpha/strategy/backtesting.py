...
class BacktestingEngine:
    ...
    def cross_order(self) -> None:
        ...
        for order in list(self.active_limit_orders.values()):
            ...
            if long_cross:
                trade_price = min(order.price, long_best_price* (1 + self.slippage))
            else:
            ...
            if trade.direction == Direction.LONG:
                trade_commission: float = max(trade_turnover * self.long_rates[trade.vt_symbol], self.min_commission)
                required_cash = trade_turnover + trade_commission
                if self.cash < required_cash:
                    # 资金不足，该订单暂不成交，保留在 active_limit_orders 中
                    continue
            else:
                trade_commission = max(trade_turnover * self.short_rates[trade.vt_symbol], self.min_commission)

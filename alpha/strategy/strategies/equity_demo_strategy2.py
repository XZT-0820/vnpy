...
class EquityDemoStrategy2(AlphaStrategy):
    ...
    def on_bars(self, bars: dict[str, BarData]) -> None: #这里的bars是回测段的全部股票！！！！
        ...
        buymore_symbols: set[str] = set(pos_symbols).difference(sell_symbols)
        ...
        buy_quantity = self.top_k - (len(pos_symbols) - len(actual_sell))
        ...
        if buy_symbols:
            ...
            for vt_symbol in buy_symbols:
                ...
                if buy_volume == 0:
                    continue
                # if buy_volume < 0:
                #     print(vt_symbol, buy_quantity, available_cash, remaining,buy_quantity)
                buyed_count += 1

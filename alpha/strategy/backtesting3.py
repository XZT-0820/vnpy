...
class BacktestingEngine3:
    ...
    def __init__(self, lab: AlphaLab) -> None:
        ...
        self.strategy: AlphaStrategy3
        self.bars: dict[str, BarData] = {} # 来作资金的收支和指标的计算等
        ...
        self.long_trade_count: int = 0  # ← 新增：买入成交次数
        self.short_trade_count: int = 0  # ← 新增：卖出成交次数
    ...
    def calculate_result(self) -> pl.DataFrame | None:
        ...
        if results:
            self.daily_df = pl.DataFrame([
                ...
                pl.Series("long_trade_count", results["long_trade_count"], dtype=pl.Int64),
                pl.Series("short_trade_count", results["short_trade_count"], dtype=pl.Int64),
    ...
    def calculate_statistics(self) -> dict:
        ...
        total_long_trade_count: int = 0
        total_short_trade_count: int = 0
        daily_trade_count: float = 0
        daily_long_trade_count: float = 0
        daily_short_trade_count: float = 0
        ...
        if positive_balance:
            ...
            total_long_trade_count = cast(int, df["long_trade_count"].sum())
            total_short_trade_count = cast(int, df["short_trade_count"].sum())
            daily_trade_count = total_trade_count / total_days
            daily_long_trade_count = total_long_trade_count / total_days
            daily_short_trade_count = total_short_trade_count / total_days
        ...
        logger.info(f"总买入笔数：  {total_long_trade_count}")
        logger.info(f"总卖出笔数：  {total_short_trade_count}")
        ...
        logger.info(f"日均买入笔数：  {daily_long_trade_count}")
        logger.info(f"日均卖出笔数：  {daily_short_trade_count}")
        ...
        statistics: dict = {
            ...
            "total_long_trade_count": total_long_trade_count,
            "total_short_trade_count": total_short_trade_count,
            "daily_trade_count": daily_trade_count,
            "daily_long_trade_count": daily_long_trade_count,
            "daily_short_trade_count": daily_short_trade_count,
    ...
    def show_performance(self, benchmark_symbol: str) -> None:
        ...
        benchmark_bars: list[BarData] = self.lab.load_bar_data(benchmark_symbol, self.interval, self.start, self.end, self.adjust_type)
    ...
    def new_bars(self, dt: datetime) -> None:
        ...
        for vt_symbol in self.vt_symbols:   #这里的symbols是数据源，是所有时间序列上的股票池的并集，并不涉及未来数据泄露，只是从数据源里获取数据
    ...
    def cross_order(self) -> None:
        """Match limit orders: sell first, then buy by signal priority"""

        # 将活跃订单分为买单和卖单
        sell_orders: list[OrderData] = []
        buy_orders: list[OrderData] = []
        ...
        # ========== 第二步：按信号顺序处理买单 ==========
        # 获取当前策略的 buy_symbols 顺序（信号从高到低）
        buy_symbols_order: list[str] = getattr(self.strategy, 'buy_symbols', [])

        # 按 buy_symbols 顺序排序买单（不在其中的放最后）
        buy_symbols_index: dict[str, int] = {s: i for i, s in enumerate(buy_symbols_order)}
        buy_orders_sorted: list[OrderData] = sorted(
            buy_orders,
            key=lambda o: buy_symbols_index.get(o.vt_symbol, float('inf'))
        )
    ...
    def process_order(self, order: OrderData) -> None:
        ...
        if not long_cross and not short_cross:
            # 价格未触发，保持 NOTTRADED 状态，继续留在 active_limit_orders 中
            return

        # 计算实际成交价：直接用 open * (1 ± slippage)
        if long_cross:
            trade_price: float = long_best_price * (1 + self.slippage)
        else:
            trade_price = short_best_price * (1 - self.slippage)
    ...
    def get_previous_signal(self) -> pl.DataFrame:
        """Get most recent signal before current datetime (yesterday's close signal)"""
        if not self.datetime:
            self.write_log("尚未开始数据回放，无法加载模型预测值")
            return pl.DataFrame()
        ...
        if signal.is_empty():
            self.write_log(f"找不到{dt}之前的信号模型预测值")
            return pl.DataFrame()

        latest_date = signal["datetime"].max()
        signal = signal.filter(pl.col("datetime") == latest_date)

        return signal

    def send_order(
        self,
        strategy: AlphaStrategy3,
        vt_symbol: str,
        direction: Direction,
        offset: Offset,
        price: float,
    ...
    def cancel_order(self, strategy: AlphaStrategy3, vt_orderid: str) -> None:
        """Cancel order"""
    ...
    def write_log(self, msg: str, strategy: AlphaStrategy3 | None = None) -> None:
        """Output log message"""
...
class PortfolioDailyResult:
    ...
    def calculate_pnl(
        ...
        for vt_symbol, contract_result in self.contract_results.items():
            ...
            self.long_trade_count += contract_result.long_trade_count
            self.short_trade_count += contract_result.short_trade_count

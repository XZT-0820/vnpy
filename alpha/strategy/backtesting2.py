...
class BacktestingEngine2:
    ...
    def cross_order(self) -> None:
        ...
        buy_symbols_order: list[str] = getattr(self.strategy, '_buy_symbols', [])
    ...
    def process_order(self, order: OrderData) -> None:
        ...
        # 计算实际成交价
        if long_cross:
            trade_price: float = min(order.price, long_best_price * (1 + self.slippage))
        else:
            trade_price = max(order.price, short_best_price * (1 - self.slippage))

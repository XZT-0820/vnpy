...
ACTIVE_STATUSES2 = set([Status.SUBMITTING, Status.NOTTRADED])
...
class OrderData(BaseData):
    ...
    def is_active(self) -> bool:
        """
        Check if the order is active. 部分成交算作active
        """
        return self.status in ACTIVE_STATUSES

    def is_active2(self) -> bool:
        """
        Check if the order is active. 部分成交不算做active
        """
        return self.status in ACTIVE_STATUSES2
...
@dataclass
class IndustryData(BaseData):
    mapping: DataFrame | None
    ...
    first_component: list[str]
    second_component: list[str]
    third_component: list[str]
